...
def jar(con):
    ...
    elif "turn on light" in con:
        speak("wich light you want to turn on")
        sleep(5)
        ans=recordAudio()
        driver = webdriver.Chrome("C:/Python24/Lib/site-packages/selenium/webdriver/chromedriver.exe")
        if "bedroom" in ans:
             driver.get("http://192.168.137.73/inbedroom") 
             driver.quit()
        elif "main room"  in ans:
            driver.get("http://192.168.137.73/inhall") 
            driver.quit()

        elif "kitchen" in ans:
            driver.get("http://192.168.137.73/inkitchen") 
            driver.quit()
        return(5)

    elif "turn off light" in con:
        speak("wich light you want to turn off")
        sleep(5)
        ans=recordAudio()
        driver = webdriver.Chrome("C:/Python24/Lib/site-packages/selenium/webdriver/chromedriver.exe")
        if "bedroom" in ans:
             driver.get("http://192.168.137.73/outbedroom") 
             driver.quit()
        elif "main room"  in ans:
            driver.get("http:///outhall") 
            driver.quit()

        elif "kitchen" in ans:
            driver.get("http://192.168.137.73/outkitchen") 
            driver.quit()
        return(5)
